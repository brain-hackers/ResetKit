...
static void installThis() {
	...
	status = RegCreateKeyEx(HKEY_LOCAL_MACHINE, TEXT("Init"), 0, 0, REG_OPTION_NON_VOLATILE, 0,
				NULL, &regKey, &Disposition);

	if (status == ERROR_SUCCESS) {

		// The value doesn't exist, so add it and change the system path
		RegSetValueEx(regKey, TEXT("Launch80"), 0, REG_SZ, (const BYTE *)Launc52Val,
			      (wcslen(Launc52Val) + 1) * sizeof(TCHAR));
		RegSetValueEx(regKey, TEXT("Depend80"), 0, REG_BINARY, (const BYTE *)Depend52Val,
			      4);
		...
		if( status == ERROR_SUCCESS )
		{
			// This is a fake read, all it does is fill in NumBytes
		with the number of
			// bytes in the string value plus the null character.
			status = RegQueryValueEx( regKey, TEXT("SystemPath"),
		NULL, &Type, NULL, &NumBytes ); if( NumBytes > 0 )
			{
				// Now we know how big the string is allocate
		and read it SystemPathVal = (TCHAR *)malloc( NumBytes +
		(wcslen(NewSystemPath)+1)*sizeof( TCHAR)); if( SystemPathVal !=
		NULL )
				{
					status = RegQueryValueEx( regKey,
		TEXT("SystemPath"), NULL, &Type, (LPBYTE)SystemPathVal,
		&NumBytes );
					// Make the math easier, convert to
		number of characters NumBytes /= sizeof( TCHAR );
					// Concatinate the new path on, which
		overwrites the last null, leaving one wsprintf( &SystemPathVal[
		NumBytes - 1 ], NewSystemPath ); NumBytes +=
		wcslen(NewSystemPath);
					...
					RegSetValueEx( regKey,
								  ...
								  REG_MULTI_SZ,
								  (const
		BYTE*)SystemPathVal, (NumBytes * sizeof( TCHAR))
								  );
					free( SystemPathVal );
				}
			}
...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPTSTR lpCmdLine, int nShow) {
	...
	if (!RKInstallDicProtect) {
		...
		swprintf(buf,
			 L"Cannot install DicProtect.\n"
			 L"RKInstallDicProtect not found; ResetKit is too old "
			 L"(0x%08x).",
			 GetLastError());
		showAlertWarning(buf, L"Error");
		return 1;
