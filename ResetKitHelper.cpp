...
typedef BOOL (*KernelIoControlProc)(DWORD dwIoControlCode, LPVOID lpInBuf, DWORD nInBufSize,
...
__attribute__((noreturn)) static void EDNA2_resetMPU() {
...
static inline bool isBatteryCoverOpen() { return HW_PINCTRL_DIN3_PIN(3) == 0; }
...
static void EDNA2_installDicProtectTimer() {
	...
	CreateThread(NULL, 16384, (LPTHREAD_START_ROUTINE)EDNA2_timerThread, NULL, 0, &tid);
	CreateThread(NULL, 16384, (LPTHREAD_START_ROUTINE)EDNA2_keyWaitThread, NULL, 0, &tid);
...
static void EDNA2_installDicProtect() {
	if (!canSoftReset()) {
		OutputDebugString(L"ResetKit: cannot install DicProtect because a soft "
				  L"reset is unavailable.");
		return;
	}
...
static int deviceGeneration() {
	...
	if (model.length() == 0) {
		...
		// Fallback to the old 2nd gen detection method to workaround
		// the issue.
...
extern "C" RESETKITHELPER_API BOOL RKH_IOControl(DWORD handle, DWORD dwIoControlCode, DWORD *pInBuf,
						 DWORD nInBufSize, DWORD *pOutBuf,
...
extern "C" BOOL APIENTRY DllMainCRTStartup(HANDLE hModule, DWORD ul_reason_for_call,
					   LPVOID lpReserved) {
	switch (ul_reason_for_call) {
	case DLL_PROCESS_ATTACH:
	case DLL_THREAD_ATTACH:

		KernelIoControl = (KernelIoControlProc)GetProcAddress(LoadLibrary(L"COREDLL"),
								      L"KernelIoControl");

		MmMapIoSpace =
		    (MmMapIoSpaceProc)GetProcAddress(LoadLibrary(L"CEDDK"), L"MmMapIoSpace");

		FileSystemPowerFunction = (FileSystemPowerFunctionProc)GetProcAddress(
		    LoadLibrary(L"COREDLL"), L"FileSystemPowerFunction");
